...
export const chainListAdapter: EntityAdapter<IChain> = createEntityAdapter<IChain>({
...
const baseChainListReducer = createReducer(
  ...
  on(chainListActions.setAllChains, (state, { chains }): EntityState<IChain> => chainListAdapter.setAll(chains, state)),
  on(chainListActions.resetChains, (state): EntityState<IChain> => chainListAdapter.removeAll(state))
);
