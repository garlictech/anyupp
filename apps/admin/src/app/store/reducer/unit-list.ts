...
const baseUnitListReducer = createReducer(
  ...
  on(unitListActions.setAllUnits, (state, { units }): EntityState<IUnit> => unitListAdapter.setAll(units, state)),
  on(unitListActions.resetUnits, (state): EntityState<IUnit> => unitListAdapter.removeAll(state))
);
