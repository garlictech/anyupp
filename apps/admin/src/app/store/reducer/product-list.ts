...
export const chainProductListAdapter: EntityAdapter<IProduct> = createEntityAdapter<IProduct>({
...
const baseChainProductListReducer = createReducer(
  ...
  on(
    ...
    (state, { products }): EntityState<IProduct> => chainProductListAdapter.setAll(products, state)
  ),
  on(productListActions.resetChainProducts, (state): EntityState<IProduct> => chainProductListAdapter.removeAll(state))
);

// GROUP PRODUCTS

export const groupProductListAdapter: EntityAdapter<IProduct> = createEntityAdapter<IProduct>({
  selectId: (item: IProduct): string => item._id,
});
...
const baseGroupProductListReducer = createReducer(
  ...
  on(
    ...
    (state, { products }): EntityState<IProduct> => groupProductListAdapter.setAll(products, state)
  ),
  on(productListActions.resetGroupProducts, (state): EntityState<IProduct> => groupProductListAdapter.removeAll(state))
);

// UNIT PRODUCTS

export const unitProductListAdapter: EntityAdapter<IProduct> = createEntityAdapter<IProduct>({
  selectId: (item: IProduct): string => item._id,
});
...
const baseUnitProductListReducer = createReducer(
  ...
  on(
    ...
    (state, { products }): EntityState<IProduct> => unitProductListAdapter.setAll(products, state)
  ),
  on(productListActions.resetUnitProducts, (state): EntityState<IProduct> => unitProductListAdapter.removeAll(state))
);

// GENERATED UNIT PRODUCTS

export const generatedUnitProductListAdapter: EntityAdapter<IProduct> = createEntityAdapter<IProduct>({
  selectId: (item: IProduct): string => item._id,
});
...
const baseGeneratedUnitProductListReducer = createReducer(
  ...
  on(
    ...
    (state, { products }): EntityState<IProduct> => generatedUnitProductListAdapter.setAll(products, state)
  ),
  on(
    productListActions.resetGeneratedUnitProducts,
    (state): EntityState<IProduct> => generatedUnitProductListAdapter.removeAll(state)
  )
);
