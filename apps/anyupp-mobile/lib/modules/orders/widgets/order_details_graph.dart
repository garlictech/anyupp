...
import 'animated_order_icon.dart';
...
class _CartDetailsGraphState extends State<CartDetailsGraph> with TickerProviderStateMixin {
  ...
  double get _orderTopPadding => _minOrderPaddingTop + (1 - _orderStateAnimation.value) * _maxOrderTopPadding;

  double get _maxOrderTopPadding => widget.height - _minOrderPaddingTop - _initialOrderPaddingBottom - _orderSize;
    ...
    return Container(
      ...
      child: Stack(
          ...
          children: <Widget>[_buildOrder()]
            ..addAll(widget.orderStates.map(_buildStateCard))
            ..addAll(widget.orderStates.map(_mapOrderStateToDot))
          // ..add(_buildFab()),
          ),
    );
  }
    ...
    return AnimatedBuilder(
      ...
      child: Column(
          children: <Widget>[
            ...
            Container(
              width: 2.0,
              height: widget.orderStates.length * OrderStateCard.height * 0.8,
              color: Colors.red, //Color.fromARGB(255, 200, 200, 200),
            ),
            SizedBox(
              height: 300.0,
            )
  ...
  _initSizeAnimations() {
    ...
    _orderSizeAnimation = Tween<double>(begin: 60.0, end: 36.0).animate(CurvedAnimation(
  ...
  void _initDotAnimations() {
    ...
    double minMarginTop = _minOrderPaddingTop + _orderSize + 0.5 * (0.8 * OrderStateCard.height);

    for (int i = 0; i < widget.orderStates.length; i++) {
