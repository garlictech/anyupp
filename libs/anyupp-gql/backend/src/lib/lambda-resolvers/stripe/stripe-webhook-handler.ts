...
const handleSuccessTransaction = (transactionId: string) => async (
  ...
  if (transaction) {
    ...
    console.log(
      '***** handleSuccessTransaction().Warning!!!! No transaction found with id=' +
        transactionId,
    );
  }
};

const handleFailedTransaction = (transactionId: string) => async (
  deps: StripeResolverDeps,
) => {
