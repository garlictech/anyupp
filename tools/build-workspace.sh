...
if [ $MODE = 'ci' ]; then
  yarn nx run-many --target=build-ci --projects=variants-manager-site --env=$ENVNAME
else
  yarn nx run-many --target=build --projects=variants-manager-site --env=$ENVNAME
fi

if [ $ENVNAME = 'dev' ]; then
  yarn nx build admin --env=$ENVNAME
elif [ $ENVNAME = 'prod' ]; then
  yarn nx build admin --configuration=production --env=$ENVNAME 
elif [ $ENVNAME = 'qa' ] || [ $ENVNAME = 'staging' ]; then
  yarn nx build admin --configuration=$ENVNAME --env=$ENVNAME
else
  yarn nx build admin --env=$ENVNAME
fi
