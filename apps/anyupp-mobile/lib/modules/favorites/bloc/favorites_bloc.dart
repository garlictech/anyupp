...
class FavoritesBloc extends Bloc<FavoritesEvent, FavoritesState> {
  ...
  Stream<FavoritesState> mapEventToState(FavoritesEvent event) async* {
    ...
    if (event is AddOrRemoveFavoriteProduct) {
      bool isFavorite = await _favoritesRepository.addOrRemoveFavoriteProduct(event.chainId, event.unitId, event.categoryId, event.productId);
    ...
    if (event is CheckProductIsFavorite) {
      ...
      bool isFavorite = await _favoritesRepository.checkIfProductIsFavorite(event.chainId, event.unitId, event.productId);
      yield ProductIsFavorite(isFavorite);
      return;
    }
  } 
