...
import { IAdminRoleEntity, IChain, IGroup, IKeyValue, IUnit } from 'src/app/shared/interfaces';
...
export class FormStaffRoleComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    combineLatest([
      ...
      .subscribe(([chains, groups, units, entities]: [IChain[], IGroup[], IUnit[], IAdminRoleEntity[]]): void => {
        // Fill the chain list
        this.chainOptions = [];
    ...
    combineLatest([
      ...
      .subscribe(([selectorValue, entities]: [any, IAdminRoleEntity[]]): void => {
        ...
        this._store
          .pipe(select(unitListSelectors.getUnitsByGroupId(selectorValue.groupId)))
          .pipe(take(1))
